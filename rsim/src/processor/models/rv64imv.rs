...
use crate::processor::isa_mods::{
    CSRProvider, IntVectorRegisterFile, IsaMod, Rv64im, Rv64imConn, Rv64v, Zicsr64, Zicsr64Conn,
};
...
impl Rv64imvProcessor {
    ...
    fn zicsr_conn<'a, 'b>(&'a mut self, rvv: &'a mut Option<Rv64v>) -> Zicsr64Conn<'b>
    where
        'a: 'b,
    {
    ...
    fn rv64im_conn<'a, 'b>(&'a mut self) -> Rv64imConn<'b>
    where
        'a: 'b,
    {
        Rv64imConn {
    ...
    fn process_inst(
        ...
        if mods.rv64im.will_handle(opcode, inst) {
            let requested_pc = mods
                .rv64im
                .execute(opcode, inst, inst_bits, self.rv64im_conn())?;
