...
use crate::processor::elements::cheri::{
    IntegerModeCheriAggregateMemory, IntegerModeCheriRV64RegisterFile,
};
...
use crate::processor::elements::cheri::{Cc128Cap, CheriAggregateMemory, CheriRV64RegisterFile};
use crate::processor::exceptions::{IllegalInstructionException, MemoryException};
use crate::processor::isa_mods::{
    CSRProvider, CheriVectorRegisterFile, IsaMod, Rv64Cheriv, Rv64im, Rv64imCapabilityMode,
    Rv64imConn, XCheri64, XCheri64Conn, Zicsr64, Zicsr64Conn,
};
...
impl CSRProvider<u64> for Rv64imvXCheriProcessorCSRs {
    ...
    fn csr_atomic_read_set(&mut self, _csr: u32, _set_bits: Option<u64>) -> Result<u64> {
        todo!()
    }
    fn csr_atomic_read_clear(&mut self, _csr: u32, _clear_bits: Option<u64>) -> Result<u64> {
        todo!()
    }
}
...
impl Rv64imvXCheriProcessor {
    ...
    fn zicsr_conn<'a, 'b>(&'a mut self, rvv: &'a mut Rv64Cheriv) -> Zicsr64Conn<'b>
    where
        'a: 'b,
    {
    ...
    fn xcheri64_conn<'a, 'b>(&'a mut self, mode: CheriExecMode) -> XCheri64Conn<'b>
    where
        'a: 'b,
    {
        XCheri64Conn {
    ...
    fn process_inst(
        ...
        if mods.xcheri.will_handle(opcode, inst) {
            let requested_pcc =
                mods.xcheri
                    .execute(opcode, inst, inst_bits, self.xcheri64_conn(mode))?;
        ...
        if mode == CheriExecMode::Capability && mods.rv64im_cap.will_handle(opcode, inst) {
            ...
            let requested_pc = {
                ...
                // rv64im.execute()
                let mut mem_wrap =
                    IntegerModeCheriAggregateMemory::wrap(&mut self.memory, self.ddc);
                mods.rv64im.execute(
                    opcode,
                    inst,
                    inst_bits,
        ...
        if mods.rvv.will_handle(opcode, inst) {
            let requested_pc = match mode {
                CheriExecMode::Capability => {
                    mods.rvv
                        .execute(opcode, inst, inst_bits, (&mut self.sreg, &mut self.memory))?
                }
                CheriExecMode::Integer => {
                    let mut reg_wrap =
                        IntegerModeCheriRV64RegisterFile::wrap(&mut self.sreg, self.ddc);
                    let mut mem_wrap =
                        IntegerModeCheriAggregateMemory::wrap(&mut self.memory, self.ddc);
                    mods.rvv
                        .execute(opcode, inst, inst_bits, (&mut reg_wrap, &mut mem_wrap))?
                }
            };
...
impl Processor<Rv64imvXCheriProcessorModules> for Rv64imvXCheriProcessor {
        ...
        let next_pcc_res: Result<Cc128Cap> = {
            ...
            if next_pcc.address() % 4 != 0 {
                Err(MemoryException::JumpMisaligned {
                    addr: next_pcc.address() as usize,
                    expected: 4,
                })?
            } else {
                Ok(next_pcc)
    ...
    fn dump(&self, mods: &Rv64imvXCheriProcessorModules) {
        println!(
            "running: {:?}\npc: {:x?}\nddc: {:x?}",
            self.running, self.pcc, self.ddc
        );
        self.sreg.dump();
        mods.rvv.dump();
    }
