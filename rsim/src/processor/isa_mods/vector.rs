...
impl<uXLEN: PossibleXlen, TElem> Rvv<uXLEN, TElem> {
    ...
    fn exec_config(
        ...
        } = inst
        {
            ...
            let vtype_bits = match inst_kind {
                ConfigKind::vsetvli => zimm11 as u64,
                ConfigKind::vsetivli => zimm10 as u64,
                ConfigKind::vsetvl => sreg.sreg_read_xlen(rs2)?.into(),
            };
            ...
            if vtype_supported {
                ...
                bail!(
                    "Valid but unsupported vtype: {:b} -> {:?}, elems_per_group {}",
                    vtype_bits,
                    req_vtype,
                    elems_per_group
                );
            ...
            unreachable!(
                "vector::exec_config instruction MUST be InstructionBits::VType, got {:?} instead",
                inst
            );
        }
    }
    ...
    fn fast_check_load_store(
        ...
        let addr_range = match op {
            Strided {
                ...
                } = self.get_active_segment_range(vm, evl).unwrap();
            ...
            Indexed {
                ...
                for i_segment in active_vstart..active_evl {
                    offsets.push(
                        self.vreg
                            .load_vreg_elem_int(index_ew, rs2, i_segment)
                            .unwrap(),
                    );
                }
        ...
        let check_result =
            sreg.check_addr_range_against_provenance(addr_range.clone(), provenance, op.dir());
        match check_result {
            Ok(()) => {
        ...
        match op {
            ...
            ByteMask { evl, .. } => {
                ...
                for i in self.vstart..evl {
                    let vec_elem = VectorElem::check_with_lmul(rd, Sew::e8, Lmul::e1, i);
                    map.push((vec_elem, addr));
                    addr += 1;
                }
            }
    ...
    fn exec_load_store(
        ...
        let max_addr = accesses
            .iter()
            .map(|(elem, addr)| addr + elem.eew.width_in_bytes())
            .max()
            .unwrap();
        ...
        match op {
            ...
            | ByteMask { dir, .. } => {
                ...
                ) in accesses
                {
                    ...
                    match dir {
                        MemOpDir::Load => self
                            .load_to_vreg(mem, eew, addr_p, base_reg, elem_within_group)
                            .with_context(|| format!("Failure on element {}", elem_within_group))?,
                        MemOpDir::Store => self
                            .store_to_mem(mem, eew, addr_p, base_reg, elem_within_group)
                            .with_context(|| format!("Failure on element {}", elem_within_group))?,
                    }
                }
            }
            FaultOnlyFirst { .. } => {
                ...
                ) in accesses
                {
                    ...
                    if elem_within_group == 0 {
                        ...
                        use crate::processor::exceptions::{CapabilityException, MemoryException};
                        ...
                        let mut error_reduces_vlen =
                            match load_err.downcast_ref::<MemoryException>() {
    ...
    fn load_to_vreg(
        ...
        self.vreg
            .store_vreg_elem(eew, vd_base, idx_from_base, val)?;
        Ok(())
    }
        ...
        match (opcode, inst) {
            (
                ...
                match funct3 {
                    ...
                    0b011 => {
                        ...
                        match funct6 {
                                ...
                                if vm == true {
                                    ...
                                    let emul = match nr {
                                        ...
                                        _ => bail!(UnsupportedParam(format!(
                                            "Invalid nr encoding in vmv<nr>r.v: nr = {}",
                                            nr
                                        ))),
                                    };
                                    ...
                                    if self.vstart >= evl {
                                        bail!(UnsupportedParam(format!(
                                            "evl {} <= vstart {} therefore vector move is no op",
                                            evl, self.vstart
                                        )))
                                    }
                                    if rd == rs2 {
                            ...
                            0b000000 => {
                                ...
                                for i in self.vstart..self.vl {
                                    if !self.vreg.seg_masked_out(vm, i) {
                                        let val = self.vreg.load_vreg_elem_int(
                                            self.vtype.vsew,
                                            rs2,
                                            i,
                                        )?;
                                        ...
                                        let val = match self.vtype.vsew {
                                            Sew::e8 => (val as u8).wrapping_add(imm as u8) as u128,
                                            Sew::e16 => {
                            ...
                            _ => bail!(MiscDecodeException(format!(
                                "Vector arithmetic funct3 {:03b} funct6 {:06b} not yet handled",
                                funct3, funct6
                            ))),
                        }
                    }
                    ...
                    _ => bail!(UnsupportedParam(format!(
                        "Vector arithmetic funct3 {:03b} currently not supported",
                        funct3
                    ))),
                }
            }
            ...
            (
                ...
                let (fast_check_result, addr_range) =
                    self.fast_check_load_store(addr_provenance, rs2, vm, op, sreg);
...
impl<uXLEN: PossibleXlen, TElem> CSRProvider<uXLEN> for Rvv<uXLEN, TElem> {
    ...
    fn csr_atomic_read_write(
        ...
        match csr {
            0xC20 | 0xC21 | 0xC22 => {
                bail!("CSR 0x{:04x} is read-only, cannot atomic read/write", csr)
            }
            _ => todo!(),
        }
    }
