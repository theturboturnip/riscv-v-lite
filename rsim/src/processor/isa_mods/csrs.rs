    ...
    T: From<u8>,
{
    ...
    fn provider_of_csr(&mut self, csr: u32) -> Option<&mut &'a mut dyn CSRProvider<T>> {
        self.csr_providers
            .iter_mut()
            .find(|provider| provider.has_csr(csr))
    }
}
...
impl<T> IsaMod<ZicsrConn<'_, T>> for Zicsr<T>
where
    T: From<u8>,
{
    type Pc = T;
                ...
                match funct3 {
                    0b001 | 0b101 => {
                        ...
                        match conn.provider_of_csr(csr) {
                            ...
                            Some(csr_provider) => {
                                let old_csr_val = csr_provider
                                    .csr_atomic_read_write(csr, need_read, write_val)?;
                    ...
                    0b010 | 0b110 => {
                        ...
                        let write_val = if rs1 == 0 { None } else { Some(rs1_val) };
                        ...
                        match conn.provider_of_csr(csr) {
                            ...
                            Some(csr_provider) => {
                                let old_csr_val =
                                    csr_provider.csr_atomic_read_set(csr, write_val)?;
                    ...
                    0b011 | 0b111 => {
                        ...
                        match conn.provider_of_csr(csr) {
                            ...
                            Some(csr_provider) => {
                                let old_csr_val =
                                    csr_provider.csr_atomic_read_clear(csr, write_val)?;
