...
use crate::processor::elements::cheri::{
    check_bounds_against_capability, check_obj_bounds_against_capability, Cc128,
    CheriAggregateMemory, CheriRV64RegisterFile, CompressedCapability,
};
use crate::processor::elements::{memory::Memory, registers::RegisterFile};
...
impl<'a, uXLEN: PossibleXlen> VecMemInterface<uXLEN, u128> for AggregateMemory {
    fn load_from_memory(&mut self, eew: Sew, addr_provenance: (u64, Provenance)) -> Result<u128> {
        ...
        let val = match eew {
            Sew::e8 => self.load_u8(addr)? as u64,
            Sew::e16 => self.load_u16(addr)? as u64,
            Sew::e32 => self.load_u32(addr)? as u64,
            Sew::e64 => self.load_u64(addr)? as u64,
            Sew::e128 => {
    ...
    fn store_to_memory(
        ...
        let (addr, _) = addr_provenance;
        match eew {
            Sew::e8 => self.store_u8(addr, val.try_into()?)?,
            Sew::e16 => self.store_u16(addr, val.try_into()?)?,
            Sew::e32 => self.store_u32(addr, val.try_into()?)?,
            Sew::e64 => self.store_u64(addr, val.try_into()?)?,
            Sew::e128 => {
...
impl<'a> VecRegInterface<u64> for CheriRV64RegisterFile {
    ...
    fn check_elem_bounds_against_provenance(
        ...
        match eew {
            Sew::e8 => check_obj_bounds_against_capability::<u8>(addr, cap, expected_perms),
            Sew::e16 => check_obj_bounds_against_capability::<u16>(addr, cap, expected_perms),
            Sew::e32 => check_obj_bounds_against_capability::<u32>(addr, cap, expected_perms),
            Sew::e64 => check_obj_bounds_against_capability::<u64>(addr, cap, expected_perms),
            Sew::e128 => check_obj_bounds_against_capability::<u128>(addr, cap, expected_perms),
        }
    }
}
...
impl<'a> VecMemInterface<u64, SafeTaggedCap> for CheriAggregateMemory {
    fn load_from_memory(
        ...
        let val = match eew {
            Sew::e8 => SafeTaggedCap::from_integer(self.load_u8(cap)? as u128),
            Sew::e16 => SafeTaggedCap::from_integer(self.load_u16(cap)? as u128),
            Sew::e32 => SafeTaggedCap::from_integer(self.load_u32(cap)? as u128),
            Sew::e64 => SafeTaggedCap::from_integer(self.load_u64(cap)? as u128),
            Sew::e128 => self.load_maybe_cap(cap)?,
        };
        Ok(val)
    }
    fn store_to_memory(
        ...
        cap.set_address_unchecked(addr);
        match eew {
            Sew::e8 => self.store_u8(cap, val.to_integer().try_into()?)?,
            Sew::e16 => self.store_u16(cap, val.to_integer().try_into()?)?,
            Sew::e32 => self.store_u32(cap, val.to_integer().try_into()?)?,
            Sew::e64 => self.store_u64(cap, val.to_integer().try_into()?)?,
            Sew::e128 => {
...
impl<'a> VecMemInterface<u64, SafeTaggedCap> for IntegerModeCheriAggregateMemory<'a> {
    fn load_from_memory(
        ...
        let (addr, _) = addr_provenance;
        let val = match eew {
            Sew::e8 => SafeTaggedCap::from_integer(self.load_u8(addr)? as u128),
            Sew::e16 => SafeTaggedCap::from_integer(self.load_u16(addr)? as u128),
            Sew::e32 => SafeTaggedCap::from_integer(self.load_u32(addr)? as u128),
            Sew::e64 => SafeTaggedCap::from_integer(self.load_u64(addr)? as u128),
            Sew::e128 => self.load_maybe_cap(addr)?,
        };
        Ok(val)
    }
    fn store_to_memory(
        ...
        let (addr, _) = addr_provenance;
        match eew {
            Sew::e8 => self.store_u8(addr, val.to_integer().try_into()?)?,
            Sew::e16 => self.store_u16(addr, val.to_integer().try_into()?)?,
            Sew::e32 => self.store_u32(addr, val.to_integer().try_into()?)?,
            Sew::e64 => self.store_u64(addr, val.to_integer().try_into()?)?,
            Sew::e128 => {
