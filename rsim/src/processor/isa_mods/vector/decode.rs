    ...
    pub fn evl(&self) -> u32 {
        ...
        match *self {
            ...
            WholeRegister { num_regs, eew, .. } => {
                num_regs as u32 * (VLEN as u32 / 8) / (eew.width_in_bytes() as u32)
            }
            ByteMask { evl, .. } => evl,
        ...
        if let InstructionBits::FLdStType { width, nf, mew, .. } = inst {
            ...
            let eew_num = match width {
                0b0001 | 0b0010 | 0b0011 | 0b0100 => {
                    bail!("LoadFP uses width for normal floats, not vectors")
                }
            ...
            unreachable!(
                "get_encoded_emul_eew_nf MUST be passed an instruction of FLdStType, got {:?}",
                inst
            )
        }
    }
    ...
    pub fn decode_load_store<uXLEN: PossibleXlen>(
        ...
        if let InstructionBits::FLdStType { rs2, mop, .. } = inst {
            ...
            let decoded_mop = match mop {
                ...
                RvvMopType::Indexed { ordered } => {
                    if eew != Sew::e128 {
                        DecodedMemOp::Indexed {
                            ordered,
                            index_ew: eew,
                            index_emul: emul,
                            eew: current_vtype.vsew,
                            emul: current_vtype.vlmul,
                            dir,
                            evl: current_vl,
                            nf,
                        }
            ...
            bail!(
                "decode_load_store MUST be passed an instruction of FLdStType, got {:?}",
                inst
            )
        }
    }
