        ...
        match (opcode, inst) {
            ...
            (
                ...
                match funct3 {
                    0b000 => conn
                        .memory
                        .store_u8(addr, (conn.sreg.read(rs2)? & 0xFF) as u8)?,
                    0b001 => conn
                        .memory
                        .store_u16(addr, (conn.sreg.read(rs2)? & 0xFFFF) as u16)?,
                    0b010 => conn
                        .memory
                        .store_u32(addr, (conn.sreg.read(rs2)? & 0xFFFF_FFFF) as u32)?,
                    0b011 => conn
                        .memory
                        .store_u64(addr, (conn.sreg.read(rs2)? & 0xFFFF_FFFF_FFFF_FFFF) as u64)?,
            ...
            (
                ...
                let new_val = match (funct7, funct3) {
                    ...
                    (1, funct3) => match funct3 {
                        ...
                        _ => bail!(UnsupportedParam(format!(
                            "Op32 MULDIV funct3: {:03b}",
                            funct3
                        ))),
                    },

                    _ => bail!(UnsupportedParam(format!(
                        "Op32 funct7/3: {:07b}, {:03b}",
                        funct7, funct3
                    ))),
                };
                conn.sreg.write(rd, (new_val as i32) as i64 as u64)?;
